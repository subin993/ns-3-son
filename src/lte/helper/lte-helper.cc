...
NetDeviceContainer
LteHelper::InstallEnbDevice (NodeContainer c,Ptr<SonServerGymEnv> son_server)
{
  NS_LOG_FUNCTION (this);
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      Ptr<Node> node = *i;
      Ptr<NetDevice> device = InstallSingleEnbDevice (node, son_server,totalN);
      devices.Add (device);
    }
  return devices;
}
...
LteHelper::InstallEnbDevice (NodeContainer c)
{
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      Ptr<Node> node = *i;
      Ptr<NetDevice> device = InstallSingleEnbDevice (node, totalN);
      devices.Add (device);
    }
  return devices;
}
...
Ptr<NetDevice>
LteHelper::InstallSingleEnbDevice (Ptr<Node> n, Ptr<SonServerGymEnv> son_server, int totalN)
{
  NS_LOG_FUNCTION (this << n);
  ...
  dev->SetTotalEnb(totalN); //NS-3 SON:
  ...
  for (std::map<uint8_t, ComponentCarrier >::iterator it = m_componentCarrierPhyParams.begin ();
       ...
       ++it)
    {
      ...
      //Ptr <FfMacScheduler> ff = CreateObject<FfMacScheduler> ();
  ...
  son_server->AddNewNode(cellId, dev);
  if (m_isAnrEnabled)
    {
...
Ptr<NetDevice>
LteHelper::InstallSingleEnbDevice (Ptr<Node> n,  int totalN)
{
  NS_LOG_FUNCTION (this << n);
  uint16_t cellId = m_cellIdCounter; // \todo Remove, eNB has no cell ID
...
//Function that calculate UEs attatched to eNB
void
LteHelper::DoCalculateUes (NetDeviceContainer enbDevices, uint16_t numOfenbs)
{
  for (uint16_t k=0; numOfenbs; k++)
  {
    enbDevices.Get(k)->GetObject<LteEnbNetDevice> ()->GetRrc ()->CalculateUes ();
  }
}
