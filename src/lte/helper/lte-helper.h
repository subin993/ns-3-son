...
public:
  ...
  ///Function that calculate UEs attached to eNB
  void DoCalculateUes (NetDeviceContainer enbDevices, uint16_t numOfenbs);
   ...
   */
  NetDeviceContainer InstallEnbDevice (NodeContainer c, Ptr<SonServerGymEnv> son_server);
...
private:
   ...
   */
  Ptr<NetDevice> InstallSingleEnbDevice (Ptr<Node> n, Ptr<SonServerGymEnv> son_server,int totalN);
  Ptr<NetDevice> InstallSingleEnbDevice (Ptr<Node> n, int totalN);
