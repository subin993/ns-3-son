...
// std::map <uint8_t, Ptr<FfMacScheduler> > LteEnbNetDevice::GetDlPrbUsageList(){
//   return m_ffmacscheduler_ns3_son;
// }



void LteEnbNetDevice::SetTotalEnb(int totalN){
  m_totalEnb = totalN;
  //std::cout<<"Print Total ENB: " << m_totalEnb << std::endl;
}
...
LteEnbNetDevice::SetMacScheduler (std::map <uint8_t, Ptr<FfMacScheduler> > ff_mac){
  ...
  for (std::map<uint8_t, Ptr<FfMacScheduler> >::iterator iter = m_ffmacscheduler_ns3_son.begin();
  iter != m_ffmacscheduler_ns3_son.end(); ++iter){
    std::cout << "[" << int(iter->first) << " " << iter->second->getDlPRBUsage() << "] ";
      ...
      if (!m_isConfigured)
        {
          ...
          m_rrc->ConfigureCell (m_ccMap,m_totalEnb);
          m_isConfigured = true;
        }
