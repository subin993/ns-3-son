...
LteUeRrc::DoDisconnect ()
{
  ...
  switch (m_state)
    ...
    default: // i.e. IDLE_RANDOM_ACCESS
      NS_LOG_UNCOND ("1212 " << ToString (m_state));
...
LteUeRrc::DoRecvRrcConnectionSetup (LteRrcSap::RrcConnectionSetup msg)
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("1313 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
}
...
LteUeRrc::DoRecvRrcConnectionReconfiguration (LteRrcSap::RrcConnectionReconfiguration msg)
{
  ...
  switch (m_state)
          ...
          if (msg.haveMeasConfig)
            {
              ...
              //DoReportUeMeasurements_fromMeasConfig();
            }
    ...
    default:
      NS_LOG_UNCOND ("1414 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
}
...
LteUeRrc::DoRecvRrcConnectionReestablishment (LteRrcSap::RrcConnectionReestablishment msg)
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("1515 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
}
...
LteUeRrc::DoRecvRrcConnectionReestablishmentReject (LteRrcSap::RrcConnectionReestablishmentReject msg)
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("1616 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
}
...
LteUeRrc::ApplyMeasConfig (LteRrcSap::MeasConfig mc)
{
  ...
  for (std::list<LteRrcSap::MeasObjectToAddMod>::iterator it = mc.measObjectToAddModList.begin ();
       ...
       ++it)
    {
      ...
      // std::cout<<"Print MeasObjectEutra: " << it->measObjectEutra.cellsToAddModList.size()<<std::endl;
      ...
      for (std::list<LteRrcSap::CellsToAddMod>::iterator iter_temp = m_var_temp.begin(); iter_temp != m_var_temp.end(); ++iter_temp){
        // std::cout <<"CellsToAddModRecievedUE "<<"["<<int(iter_temp->cellIndex)<<" "<<int(iter_temp->cellIndividualOffset) <<"]"<<std::endl;
      }
    ...
    std::list<LteRrcSap::CellsToAddMod> m_var_temp = m_varMeasConfig.measObjectList[1].measObjectEutra.cellsToAddModList;
    
    for (std::list<LteRrcSap::CellsToAddMod>::iterator iter_temp = m_var_temp.begin(); iter_temp != m_var_temp.end(); ++iter_temp){
      // std::cout <<"CellsToAddModUE "<<"["<<int(iter_temp->cellIndex)<<" "<<int(iter_temp->cellIndividualOffset) <<"]"<<std::endl;
    }
...
LteUeRrc::MeasurementReportTriggering (uint8_t measId)
{
  ...
  switch (reportConfigEutra.eventId)
    ...
    case LteRrcSap::ReportConfigEutra::EVENT_A3:
      {
        // std::cout<<"A3 Event Handover Measurement Report PRINT "<<int(measId)<<std::endl;
        /*
        ...
        for (std::map<uint16_t, MeasValues>::iterator storedMeasIt = m_storedMeasValues.begin ();
             ...
             ++storedMeasIt)
          {
            ...
            for (std::list<LteRrcSap::CellsToAddMod>::iterator iter_temp = m_var_temp.begin(); 
            iter_temp != m_var_temp.end(); ++iter_temp){
              // std::cout <<"CellsToAddModUE "<<"["<<int(iter_temp->cellIndex)<<" "<<int(iter_temp->cellIndividualOffset) <<"]"<<std::endl;
              if (iter_temp->cellIndex == m_cellId){
                ocp = iter_temp->cellIndividualOffset;
              }
              else if (iter_temp->cellIndex == cellId){
                ocn = iter_temp->cellIndividualOffset;
              }
            ...
            bool hasTriggered = isMeasIdInReportList
              && (measReportIt->second.cellsTriggeredList.find (cellId)
                  != measReportIt->second.cellsTriggeredList.end ());
            // std::cout<<"ocp ocn:" << ocp << " " << ocn << std::endl;
