...
UeManager::InitialContextSetupRequest ()
{
  ...
  if (m_state == ATTACH_REQUEST)
    ...
    {
      NS_LOG_UNCOND ("11 " << ToString (m_state));
...
void
UeManager::ScheduleRrcConnectionRecursive(){
  // std::cout<<"ScheduleRrcConnectionReconfiguration"<<std::endl;
  if (m_state == CONNECTED_NORMALLY || m_state==CONNECTION_RECONFIGURATION){
...
UeManager::ScheduleRrcConnectionReconfiguration ()
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("22 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
}
...
UeManager::PrepareHandover (uint16_t cellId)
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("33 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
...
UeManager::SendData (uint8_t bid, Ptr<Packet> p)
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("44 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
}
...
UeManager::SendUeContextRelease ()
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("55 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
}
...
UeManager::RecvHandoverPreparationFailure (uint16_t cellId)
{
  ...
  switch (m_state)
    {
    case HANDOVER_PREPARATION:
      // NS_LOG_UNCOND ("temptemptemptemptemptemp");
      // NS_LOG_UNCOND (cellId);
      // NS_LOG_UNCOND (m_targetCellId);
    ...
    default:
      NS_LOG_UNCOND ("66 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
}
...
{
  NS_LOG_FUNCTION (this);
      NS_LOG_UNCOND ("77 " << ToString (m_state));
...
UeManager::RecvRrcConnectionRequest (LteRrcSap::RrcConnectionRequest msg)
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("88 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
}
...
UeManager::RecvRrcConnectionSetupCompleted (LteRrcSap::RrcConnectionSetupCompleted msg)
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("99 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
}
...
UeManager::RecvRrcConnectionReconfigurationCompleted (LteRrcSap::RrcConnectionReconfigurationCompleted msg)
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("1010 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
}
...
UeManager::RecvRrcConnectionReestablishmentRequest (LteRrcSap::RrcConnectionReestablishmentRequest msg)
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("1111 " << ToString (m_state));
      NS_FATAL_ERROR ("method unexpected in state " << ToString (m_state));
      break;
    }
...
UeManager::BuildRrcConnectionReconfiguration ()
{
  ...
  //std::cout<<"PRINT M_RRC_DL_BANDWIDTH" << m_rrc->m_dlBandwidth << std::endl;
  ...
  // for (int i =1; i <= totalEnb; i++){
  //   LteRrcSap::CellsToAddMod cellsToAddMod;
  //   cellsToAddMod.cellIndex=i;
  //   cellsToAddMod.cellIndividualOffset=0;
  //   cellsToAddMod.physCellId=0;
  //   measObject.measObjectEutra.cellsToAddModList.push_back(cellsToAddMod);
  // }
  // //measObject.measObjectEutra.CellstoAddModList.push_back ();

  
  // m_rrc->m_ueMeasConfig.quantityConfig.filterCoefficientRSRP = m_rrc->m_rsrpFilterCoefficient;
  // m_rrc->m_ueMeasConfig.quantityConfig.filterCoefficientRSRQ = m_rrc->m_rsrqFilterCoefficient;
  // m_rrc->m_ueMeasConfig.haveMeasGapConfig = false;
  // m_rrc->m_ueMeasConfig.haveSmeasure = false;
  // m_rrc->m_ueMeasConfig.haveSpeedStatePars = false;
  //


  //LteRrcSap::MeasObjectEutra measObjectEutra;
  measObject.measObjectEutra.cellsToAddModList = m_rrc->m_cellsToAddModList;
  //m_rrc->m_ueMeasConfig.measObjectToAddModList
  // std::cout<<"PRINT size of measobjecttoaddmod" << m_rrc->m_ueMeasConfig.measObjectToAddModList.size() << std::endl;
  // std::cout<<"PRINT size of measobjecttoaddmod" << msg.measConfig.measObjectToAddModList.size() << std::endl;
  //m_rrc->m_ueMeasConfig.measObjectToAddModList.push_back(measObjectToAddMod);
  //
  msg.measConfig = m_rrc->m_ueMeasConfig;

  msg.measConfig.measObjectToAddModList.push_back(measObject);
  
  // for (std::list<LteRrcSap::MeasObjectToAddMod>::iterator it = msg.measConfig.measObjectToAddModList.begin ();
  //      it !=  msg.measConfig.measObjectToAddModList.end ();
  //      ++it)
  // {
  //   std::list<LteRrcSap::CellsToAddMod> m_var_temp = it->measObjectEutra.cellsToAddModList;
  //   for (std::list<LteRrcSap::CellsToAddMod>::iterator iter_temp = m_var_temp.begin(); iter_temp != m_var_temp.end(); ++iter_temp){
  //   std::cout <<"CellsToAddModEnb "<<"["<<int(iter_temp->cellIndex)<<" "<<int(iter_temp->cellIndividualOffset) <<"]"<<std::endl;
  //   }
  // }
  // std::list<LteRrcSap::CellsToAddMod> m_var_temp = measObject.measObjectEutra.cellsToAddModList;
...
void
LteEnbRrc::ConfigureCell (std::map<uint8_t, Ptr<ComponentCarrierBaseStation>> ccPhyConf,int totalEnb)
{
  std::cout<<"configurecell"<<std::endl;
  std::list<LteRrcSap::CellsToAddMod> temp_list;
  for(int i = 1; i <=totalEnb; i++){ //for NS-3 SON  
    LteRrcSap::CellsToAddMod tempAddMod;
    tempAddMod.cellIndex=i;
    tempAddMod.physCellId=0;
    tempAddMod.cellIndividualOffset=0;
    temp_list.push_back(tempAddMod);
  }
  // m_cellsToAddModList = temp_list;
  setCellstoAddModList(temp_list);
...
LteEnbRrc::RemoveUe (uint16_t rnti)
{
  ...
  //uint8_t componentCarrierId = it->second->GetComponentCarrierId();
  //const uint16_t cellId = ComponentCarrierToCellId (componentCarrierId);
...
//Calculate the number of UEs attatched to eNB
void
LteEnbRrc::CalculateUes ()
{
  NS_LOG_UNCOND("Number of UEs" << m_ueMap.size());
}
...
void LteEnbRrc::setCellstoAddModList(std::list<LteRrcSap::CellsToAddMod> list_temp){
  m_cellsToAddModList = list_temp;
}
